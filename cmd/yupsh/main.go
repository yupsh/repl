...
func run(ctx context.Context, fs afero.Fs, home expansion.Home) error {
	...
	return runInteractive(ctx, terminalFD(fd), fs, home)
}

// terminalFD is the file descriptor of the controlling terminal (stdin) that
// the interactive session puts into raw mode.
type terminalFD int
...
// correctly.
func runInteractive(ctx context.Context, fd terminalFD, fs afero.Fs, home expansion.Home) error {
