...
func buildEcho(_ afero.Fs, positional flags.Argv, _ []any) (Segment, error) {
	words := positional.Strings()
	texts := make([]echo.Text, len(words))
	for i, w := range words {
		texts[i] = echo.Text(w)
	}
	return Segment{Source: echo.Echo(texts...)}, nil
}
...
func buildLs(fs afero.Fs, positional flags.Argv, opts []any) (Segment, error) {
	...
	return Segment{Source: ls.Ls(gloo.File(flags.FirstOr(positional, ".")), opts...)}, nil
}
...
func buildFind(fs afero.Fs, positional flags.Argv, opts []any) (Segment, error) {
	...
	src := find.Find(gloo.File(flags.FirstOr(positional, ".")),
		find.FindFs{Fs: fs},
		find.FindName(name),
		find.FindType(typ),
...
func buildGrep(_ afero.Fs, positional flags.Argv, opts []any) (Segment, error) {
	...
	cmd := grep.Grep(grep.GrepPattern(string(positional[0])), opts...)
...
func buildSed(_ afero.Fs, positional flags.Argv, _ []any) (Segment, error) {
	...
	return Segment{Command: sed.Sed(sed.SedScript(string(positional[0]))), Files: toFiles(positional[1:])}, nil
}
...
func buildTr(_ afero.Fs, positional flags.Argv, opts []any) (Segment, error) {
	...
	return Segment{Command: tr.Tr(tr.TrSet(from), tr.TrSet(to), opts...), Files: toFiles(rest)}, nil
}
...
func buildShuf(_ afero.Fs, positional flags.Argv, opts []any) (Segment, error) {
	...
	if echo {
		words := positional.Strings()
		lines := make([]shuf.ShufLine, len(words))
		for i, w := range words {
			lines[i] = shuf.ShufLine(w)
		}
		clean = append(clean, shuf.ShufEcho(lines...))
...
// present.
func extractEcho(opts []any) (clean []any, isEcho bool) {
