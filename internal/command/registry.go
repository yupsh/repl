...
func Registry() map[Name]Builder {
	return map[Name]Builder{
		nameEcho: {IsRaw: true, Build: buildEcho, Summary: "emit arguments as a line (source)"},
		nameEmit: {IsRaw: true, Build: buildEmit, Summary: "emit literal text (source)"},
		nameYes:  {IsRaw: true, Build: buildYes, Summary: "repeat a line until interrupted (source)"},
		...
		nameGit: {
			IsRaw: true,
			Build: command(
				func(p flags.Argv, _ []any) gloo.Command[[]byte, []byte] {
					ss := p.Strings()
					args := make([]git.GitArg, len(ss))
					for i, s := range ss {
						args[i] = git.GitArg(s)
					}
					return git.Git(args...)
		...
		namePerl: {
			Flags: perlFlags,
			Build: command(
				func(p flags.Argv, o []any) gloo.Command[[]byte, []byte] {
					return perl.Perl(append(p.Anys(), o...)...)
				},
			),
...
var (
	...
	cutFlags = flags.Set{
		...
		flags.Value("f", "fields", func(v flags.Argument) (any, error) {
			...
			fields := make([]cut.CutField, len(xs))
			for i, x := range xs {
				fields[i] = cut.CutField(x)
			}
			return cut.CutFields(fields...), nil
		}),
	...
	shufFlags = flags.Set{
		...
		flags.Value("i", "input-range", func(v flags.Argument) (any, error) {
			...
			return shuf.ShufRange(shuf.ShufBound(lo), shuf.ShufBound(hi)), nil
		}),
		flags.Bool("e", flags.LongFlag(nameEcho), shufEchoMarker{}),
	}
