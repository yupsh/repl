...
func TestTildeExpand(t *testing.T) {
	...
	for _, c := range cases {
		if got := tildeExpand(word(c.word), Home(c.home)); got != word(c.want) {
...
func TestHasGlobMeta(t *testing.T) {
	for _, w := range []word{"*.go", "a?b", "x[0-9]"} {
		if !hasGlobMeta(w) {
			t.Errorf("hasGlobMeta(%q) = false", w)
		}
	}
	for _, w := range []word{"plain", "a.go", "~/x"} {
		if hasGlobMeta(w) {
...
func TestExpand(t *testing.T) {
	...
	cases := []struct {
		...
		{"quoted literal", []token.Token{{Text: "/p/*.go", IsQuoted: true}}, []string{"/p/*.go"}},
