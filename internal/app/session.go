...
// logic: each input line is handed to the line orchestrator (internal/line),
...
import (
	...
	"github.com/yupsh/repl/internal/line"
...
func NewWithReader(reader LineReader, stdin io.Reader, out, errw io.Writer, fs afero.Fs, home expansion.Home) Session {
	return Session{
		...
		out:    out,
		errw:   errw,
		cfg:    line.Config{Fs: fs, Home: home},
	}
}
...
func (e Session) dispatch(ctx context.Context, line token.Line) (isStop bool) {
...
func (e Session) builtin(line token.Line) (isHandled, isStop bool) {
...
// execute plans one pipeline line via the line orchestrator, then streams the
// assembled result to the output writer.
func (e Session) execute(ctx context.Context, input token.Line) error {
	asm, err := line.Run(e.cfg, e.stdin, input)
	if err != nil {
		return err
	}
...
// format is a fmt.Fprintf-style format string for the session's output helpers.
type format string
...
func fprintf(w io.Writer, f format, a ...any) {
	_, _ = fmt.Fprintf(w, string(f), a...)
}
